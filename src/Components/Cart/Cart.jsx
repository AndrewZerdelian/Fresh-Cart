      ...
      {IsLoading ? (
        ...
        <div className="pt-2">
          <div className="d-flex justify-content-start align-items-center pb-3 container">
            <button className="btn btn-danger w-25" onClick={GETClearAllUserCart}>
              DELETE ALL
            </button>
          </div>
            ...
            {CartDetails.data.products.map((product) => (
              <div key={product.product.id} className="row border-bottom py-2 px-2">
                ...
                <div className="col-md-11">
                  ...
                  <button onClick={() => DeletedItems(product.product.id)} className="btn p-0">
                    <i className="text-danger fas fa-trash-alt"></i> Remove Item
                  </button>
                </div>
              </div>
            ))}
            <div className="d-flex justify-content-between pt-3">
              <Link to={`/Address`} className="btn bg-main text-white fw-bold w-25">
                Online Payment
              </Link>
